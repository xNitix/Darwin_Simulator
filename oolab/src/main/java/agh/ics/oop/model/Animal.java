...
public class Animal implements WorldElement {
    private static int nextId = 0; // zmienna wspoldzielona (static) // ten komentarz nic nie wnosi
    ...
    public int grassEatenCounter = 0; // public?
    ...
    public Animal(Vector2d position, int[] genoType, int currentEnergy, Boolean madness) {
        this.id = nextId;
        nextId++;
        Random random = new Random(); // ten z atrybutu się nie nada?
    ...
    public void move(int energyCost, Vector2d newPosition) {
        ...
        if (random.nextDouble() <= randomFactor) { // warianty załatwiamy if'em?
            whichGenIsActive++;
    ...
    public void addSomeMadness() {
        randomFactor = 0.8;
    } // Ustawienie 80% szans na standardową aktywację, 20% szans na losowy gen (dodatek 3)
    ...
    public int getWhichGenIsActive() {
        return whichGenIsActive;
    }
